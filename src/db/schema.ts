...
export const productTemplates = pgTable(
  'product_templates',
  {
    ...
    orgId: text('org_id')
      .notNull()
      .references(() => organization.id, { onDelete: 'cascade' }),
    businessTemplateId: text('business_template_id').references(
      () => businessTemplates.id,
      { onDelete: 'set null' },
    ),
    businessTemplateItemKey: text('business_template_item_key'),
    name: text('name').notNull(),
    description: text('description'),
  ...
  (table) => [
    index('idx_product_templates_org_id').on(table.orgId),
    index('idx_product_templates_business_template_id').on(
      table.businessTemplateId,
    ),
    uniqueIndex('idx_product_templates_org_business_template_item').on(
      table.orgId,
      table.businessTemplateId,
      table.businessTemplateItemKey,
    ),
    uniqueIndex('idx_product_templates_id_org_id').on(table.id, table.orgId),
  ],
)
...
export const products = pgTable(
  'products',
  {
    ...
    orgId: text('org_id')
      .notNull()
      .references(() => organization.id, { onDelete: 'cascade' }),
    productTemplateId: text('product_template_id').references(
      () => productTemplates.id,
      { onDelete: 'set null' },
    ),
    itemizationMode: text('itemization_mode').notNull().default('uniform'),
    configuration: json('configuration').$type<ProductTemplateConfiguration>(),
    name: text('name').notNull(),
    description: text('description'),
    active: boolean('active').notNull().default(true),
