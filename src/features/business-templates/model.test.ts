...
import { getOrderCreationReadiness } from '#/features/orders/model'
import { listProductFields } from '#/features/product-configuration/model'
import { listStages } from '#/features/production/model'
...
describe('materialization', () => {
  ...
  it('supports runtime provenance links to configuration elements and null links for manual rows', async () => {
    await materializeTemplate(orgId, templateId)
    const elements = await listConfigurationElements(orgId)
    const productElement = elements.find((e) => e.elementType === 'product')
    ...
    const manualProduct = await db
      ...
      .values({
        ...
        orgId,
        name: 'Manual Product',
        configurationElementId: null,
      })
      .returning()
    expect(manualProduct[0]?.configurationElementId).toBeNull()

    const createdManualProduct = manualProduct[0]
    expect(createdManualProduct).toBeDefined()
    if (!createdManualProduct) return
    ...
    const manualField = await db
      ...
      .values({
        id: 'manual-field-1',
        orgId,
        productId: createdManualProduct.id,
        fieldType: 'select',
      ...
      .returning()
    expect(manualField[0]?.configurationElementId).toBeNull()

    const manualStage = await db
      ...
      .returning()
    expect(manualStage[0]?.configurationElementId).toBeNull()
    ...
    const manualPayment = await db
      ...
      .returning()
    expect(manualPayment[0]?.configurationElementId).toBeNull()

    const selectedByElement = await db
      .select()
      .from(products)
      .where(eq(products.configurationElementId, productElement.id))

    expect(selectedByElement).toHaveLength(1)
    expect(selectedByElement[0]?.configurationElementId).toBe(productElement.id)
  })
})
  ...
  beforeEach(async () => {
    ...
    const v2 = await createTemplate({
      ...
      configSnapshot: {
        ...
        regionalDefaults: { timezone: 'Asia/Jakarta' },
        fulfillmentDefaults: { shippingEnabled: true },
        pricingConfig: { defaultMode: 'step' },
      },
    })
...
// ─── Task 2 Template Runtime Projection ─────────────────────────────────────

describe('Task 2 template runtime projection', () => {
  it('materializes apparel products including Jersey Screen Print and T-Shirt DTF Print', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return

    await materializeTemplate(orgId, apparel.id)
    ...
    const productNames = orgProducts.map((p) => p.name)
    expect(productNames).toContain('Jersey Screen Print')
    expect(productNames).toContain('T-Shirt DTF Print')
  })

  it('materializes apparel stages including Artwork Review, Sample Approval, and Decoration Production', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return
    ...
    const stageNames = orgStages.map((s) => s.name)
    expect(stageNames).toContain('Artwork Review')
    expect(stageNames).toContain('Sample Approval')
    expect(stageNames).toContain('Decoration Production')
  })

  it('materializes rubber accessories with no apparel products or stages', async () => {
    const published = await listPublishedTemplates()
    const rubber = published.find((t) => t.slug === 'rubber-accessories')
    expect(rubber).toBeDefined()
    if (!rubber) return

    await materializeTemplate(orgId2, rubber.id)

    const orgProducts = await db
      .select()
      .from(products)
      .where(eq(products.orgId, orgId2))

    const productNames = orgProducts.map((p) => p.name)
    expect(productNames).not.toContain('Jersey Screen Print')
    expect(productNames).not.toContain('T-Shirt DTF Print')

    const orgStages = await db
      .select()
      .from(productionStages)
      .where(eq(productionStages.orgId, orgId2))

    const stageNames = orgStages.map((s) => s.name)
    expect(stageNames).not.toContain('Artwork Review')
    expect(stageNames).not.toContain('Sample Approval')
    expect(stageNames).not.toContain('Decoration Production')
  })

  it('links projected products and stages to inherited configuration elements', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return
    ...
    expect(orgProducts.length).toBeGreaterThan(0)
    for (const prod of orgProducts) {
      expect(prod.configurationElementId).not.toBeNull()
      if (!prod.configurationElementId) continue
      const element = await db
        ...
        .where(
          and(
            ...
            eq(configurationElements.id, prod.configurationElementId),
          ),
        )
        .limit(1)
      expect(element[0]).toBeDefined()
      expect(element[0]?.provenance).toBe('inherited')
      expect(element[0]?.elementType).toBe('product')
    }
    ...
    for (const stage of orgStages) {
      expect(stage.configurationElementId).not.toBeNull()
      if (!stage.configurationElementId) continue
      const element = await db
        .select()
        .from(configurationElements)
        .where(
          and(
            eq(configurationElements.orgId, orgId),
            eq(configurationElements.id, stage.configurationElementId),
          ),
        )
        .limit(1)
      expect(element[0]).toBeDefined()
      expect(element[0]?.provenance).toBe('inherited')
      expect(element[0]?.elementType).toBe('workflow_stage')
    }
  })

  it('returns configured fields via listProductFields', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return
    ...
    const [jerseyProduct] = await db
      .select()
      .from(products)
      .where(
        and(
          eq(products.orgId, orgId),
          eq(products.name, 'Jersey Screen Print'),
        ),
      )
      .limit(1)

    expect(jerseyProduct).toBeDefined()
    if (!jerseyProduct) return

    const fields = await listProductFields(orgId, jerseyProduct.id)
    expect(fields.length).toBeGreaterThan(0)
    const fieldKeys = fields.map((f) => f.fieldKey)
    expect(fieldKeys).toContain('print_location')
    expect(fieldKeys).toContain('size_matrix')
    expect(fieldKeys).toContain('artwork_file')

    for (const f of fields) {
      expect(f.configurationElementId).not.toBeNull()
    }
  })

  it('creates an active default bank transfer payment method', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return
    ...
    expect(methods[0]?.type).toBe('bank_transfer')
    expect(methods[0]?.configurationElementId).not.toBeNull()
  })

  it('prevents double materialization and does not duplicate runtime rows', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    expect(apparel).toBeDefined()
    if (!apparel) return
    ...
    const initialProducts = await db
      ...
      .from(products)
      .where(eq(products.orgId, orgId))
    const initialStages = await db
      .select()
      .from(productionStages)
      .where(eq(productionStages.orgId, orgId))
    const initialFields = await db
      .select()
      .from(productFields)
      .where(eq(productFields.orgId, orgId))
    const initialPaymentMethods = await db
      .select()
      .from(paymentMethods)
      .where(eq(paymentMethods.orgId, orgId))

    await expect(materializeTemplate(orgId, apparel.id)).rejects.toThrow(
      'already has a configuration',
    )
    ...
    const finalStages = await db
      ...
      .from(productionStages)
      .where(eq(productionStages.orgId, orgId))
    const finalFields = await db
      .select()
      .from(productFields)
      .where(eq(productFields.orgId, orgId))
    const finalPaymentMethods = await db
      .select()
      .from(paymentMethods)
      .where(eq(paymentMethods.orgId, orgId))

    expect(finalProducts.length).toBe(initialProducts.length)
    expect(finalStages.length).toBe(initialStages.length)
    expect(finalFields.length).toBe(initialFields.length)
    expect(finalPaymentMethods.length).toBe(initialPaymentMethods.length)
  })
})
// ─── Task 5 End-to-End Template Acceptance Path ──────────────────────────────

describe('Task 5 end-to-end template-selected acceptance path', () => {
  it('verifies published apparel and rubber templates with isolation, typed fields, order readiness, and non-duplication', async () => {
    const published = await listPublishedTemplates()
    const apparel = published.find((t) => t.slug === 'apparel-decoration')
    const rubber = published.find((t) => t.slug === 'rubber-accessories')
    ...
    expect(apparel).toBeDefined()
    expect(rubber).toBeDefined()
    if (!apparel || !rubber) return

    // 1. Materialize templates for distinct organizations
    const apparelResult = await materializeTemplate(orgId, apparel.id)
    const rubberResult = await materializeTemplate(orgId2, rubber.id)

    expect(apparelResult.elementsCreated).toBeGreaterThan(0)
    expect(rubberResult.elementsCreated).toBeGreaterThan(0)

    // 2. Apparel Organization - Public API Verification
    const apparelProducts = await listProducts({ orgId })
    const apparelProductNames = apparelProducts.map((p) => p.name)
    expect(apparelProductNames).toContain('Jersey Screen Print')
    expect(apparelProductNames).toContain('T-Shirt DTF Print')

    const apparelStages = await listStages(orgId)
    const apparelStageNames = apparelStages.map((s) => s.name)
    expect(apparelStageNames).toContain('Artwork Review')
    expect(apparelStageNames).toContain('Sample Approval')
    expect(apparelStageNames).toContain('Decoration Production')

    // Typed product fields for apparel
    const jerseyProduct = apparelProducts.find(
      (p) => p.name === 'Jersey Screen Print',
    )
    expect(jerseyProduct).toBeDefined()
    if (jerseyProduct) {
      const jerseyFields = await listProductFields(orgId, jerseyProduct.id)
      const fieldKeys = jerseyFields.map((f) => f.fieldKey)
      expect(fieldKeys).toContain('print_location')
      expect(fieldKeys).toContain('size_matrix')
      expect(fieldKeys).toContain('artwork_file')
      expect(fieldKeys).not.toContain('mold_type')
      expect(fieldKeys).not.toContain('attachment')
    }

    // Readiness for apparel org
    const apparelReadinessInitial = await getOrderCreationReadiness(orgId)
    expect(apparelReadinessInitial.activeProductCount).toBeGreaterThan(0)
    expect(apparelReadinessInitial.productionStageCount).toBeGreaterThan(0)
    expect(apparelReadinessInitial.paymentMethodCount).toBe(1)
    expect(apparelReadinessInitial.completedCount).toBe(3)

    // Complete business address prerequisite to achieve full order creation readiness
    const addressId = 'addr-00000000-0000-0000-0000-000000000001'
    const profileId = 'prof-00000000-0000-0000-0000-000000000001'
    await db.insert(addressesTable).values({
      id: addressId,
      orgId,
      areaId: 'area-jakarta-south',
    ...
    await db.insert(orgProfilesTable).values({
      ...
      orgId,
      addressId,
      displayName: 'Apparel Corp',
    })

    const apparelReadinessFinal = await getOrderCreationReadiness(orgId)
    expect(apparelReadinessFinal.businessAddressComplete).toBe(true)
    expect(apparelReadinessFinal.isReady).toBe(true)
    expect(apparelReadinessFinal.completedCount).toBe(4)

    // 3. Rubber Organization - Public API Verification
    const rubberProducts = await listProducts({ orgId: orgId2 })
    const rubberProductNames = rubberProducts.map((p) => p.name)
    expect(rubberProductNames).toContain('Rubber Keychain')
    expect(rubberProductNames).toContain('Rubber Wristband')
    expect(rubberProductNames).toContain('Rubber Patch')

    const rubberStages = await listStages(orgId2)
    const rubberStageNames = rubberStages.map((s) => s.name)
    expect(rubberStageNames).toContain('Design Review')
    expect(rubberStageNames).toContain('Mold Preparation')
    expect(rubberStageNames).toContain('Production')
    expect(rubberStageNames).toContain('Quality Check')

    // Typed product fields for rubber
    const keychainProduct = rubberProducts.find(
      (p) => p.name === 'Rubber Keychain',
    )
    expect(keychainProduct).toBeDefined()
    if (keychainProduct) {
      const keychainFields = await listProductFields(orgId2, keychainProduct.id)
      const fieldKeys = keychainFields.map((f) => f.fieldKey)
      expect(fieldKeys).toContain('mold_type')
      expect(fieldKeys).toContain('attachment')
      expect(fieldKeys).toContain('artwork_file')
      expect(fieldKeys).not.toContain('print_location')
      expect(fieldKeys).not.toContain('size_matrix')
    }

    const rubberReadiness = await getOrderCreationReadiness(orgId2)
    expect(rubberReadiness.activeProductCount).toBeGreaterThan(0)
    expect(rubberReadiness.productionStageCount).toBeGreaterThan(0)
    expect(rubberReadiness.paymentMethodCount).toBe(1)

    // 4. Cross-Organization Isolation
    // Apparel organization has no rubber products or stages
    expect(apparelProductNames).not.toContain('Rubber Keychain')
    expect(apparelProductNames).not.toContain('Rubber Wristband')
    expect(apparelProductNames).not.toContain('Rubber Patch')
    expect(apparelStageNames).not.toContain('Mold Preparation')

    // Rubber organization has no apparel products or stages
    expect(rubberProductNames).not.toContain('Jersey Screen Print')
    expect(rubberProductNames).not.toContain('T-Shirt DTF Print')
    expect(rubberStageNames).not.toContain('Artwork Review')
    expect(rubberStageNames).not.toContain('Sample Approval')
    expect(rubberStageNames).not.toContain('Decoration Production')

    // 5. Idempotency & Row Non-Duplication
    await expect(materializeTemplate(orgId, apparel.id)).rejects.toThrow(
      'already has a configuration',
    )

    const apparelProductsAfterRetry = await listProducts({ orgId })
    const apparelStagesAfterRetry = await listStages(orgId)

    expect(apparelProductsAfterRetry.length).toBe(apparelProducts.length)
    expect(apparelStagesAfterRetry.length).toBe(apparelStages.length)
  })
})
