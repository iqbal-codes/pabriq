import type { Messages } from './types'

// biome-ignore format: locale message map is maintained as a dense dictionary
const id: Messages = {
  auth: {
    signIn: "Masuk",
    signUp: "Buat akun",
    email: "Email",
    password: "Kata sandi",
    name: "Nama",
    signInTitle: "Masuk",
    signUpTitle: "Buat akun",
    signInDesc: "Masukkan email dan kata sandi untuk melanjutkan.",
    signUpDesc: "Gunakan email dan kata sandi untuk membuat akun.",
    alreadyHaveAccount: "Sudah punya akun?",
    needAccount: "Butuh akun?",
    createOne: "Buat satu",
    nameMin: "Nama harus minimal 2 karakter",
    emailValid: "Masukkan alamat email yang valid",
    passwordMin: "Kata sandi harus minimal 8 karakter",
    passwordDesc: "Minimal 8 karakter.",
    authFailed: "Autentikasi gagal",
    forgotPassword: "Lupa kata sandi?",
    forgotPasswordTitle: "Lupa kata sandi?",
    forgotPasswordDesc: "Masukkan email Anda dan kami akan mengirimkan tautan reset kata sandi.",
    sendResetLink: "Kirim tautan reset",
    resetLinkSent: "Periksa email Anda",
    resetLinkSentDesc: "Jika akun dengan email tersebut ada, kami telah mengirimkan tautan reset kata sandi.",
    backToSignIn: "Kembali ke halaman masuk",
    resetPassword: "Reset kata sandi",
    resetPasswordTitle: "Pilih kata sandi baru",
    resetPasswordDesc: "Masukkan kata sandi baru untuk akun Pabriq Anda.",
    newPassword: "Kata sandi baru",
    confirmPassword: "Konfirmasi kata sandi",
    passwordMismatch: "Kata sandi tidak cocok",
    passwordResetSuccess: "Reset kata sandi selesai",
    passwordResetSuccessDesc: "Kata sandi Anda telah diperbarui. Sekarang Anda dapat masuk.",
    invalidResetLink: "Tautan reset kata sandi ini tidak valid atau sudah kedaluwarsa.",
    verificationPendingTitle: "Periksa kotak masuk Anda",
    verificationPendingDesc: "Kami telah mengirimkan tautan verifikasi. Buka tautan tersebut untuk memverifikasi alamat email Anda.",
    verificationSent: "Email verifikasi terkirim",
    verificationSentDesc: "Jika email ini terdaftar pada akun, tautan verifikasi sedang dikirim.",
    resendVerification: "Kirim ulang email verifikasi",
    emailVerifiedTitle: "Email terverifikasi",
    emailVerifiedDesc: "Alamat email Anda telah terverifikasi. Lanjutkan ke Pabriq.",
    invalidVerificationLink: "Tautan verifikasi ini tidak valid.",
    verificationExpired: "Tautan verifikasi ini telah kedaluwarsa.",
    emailNotVerified: "Verifikasi alamat email Anda sebelum masuk.",
    checkInbox: "Periksa kotak masuk Anda",
    continueToApp: "Lanjutkan ke Pabriq",
  },
  admin: {
    logOut: "Keluar",
    controlPanel: "Panel Kontrol",
    backToApp: "Kembali ke Aplikasi",
    dashboard: "Dasbor",
    totalOrganizations: "Total Organisasi",
    activeSubscriptions: "Langganan Aktif",
    trialingOrgs: "Uji Coba",
    pendingMigrations: "Migrasi Tertunda",
    ordersThisMonth: "Pesanan Bulan Ini",
    unpaidInvoicesTotal: "Total Faktur Belum Dibayar",
    productionBottlenecks: "Hambatan Produksi",
    qualityHolds: "Penahanan Kualitas",
    recentSignups: "Pendaftaran Baru",
    organizations: "Organisasi",
    searchOrgs: "Cari organisasi...",
    orgName: "Organisasi",
    members: "Anggota",
    subscription: "Langganan",
    plan: "Paket",
    orders: "Pesanan",
    unpaidInvoices: "Faktur Belum Dibayar",
    actions: "Tindakan",
    suspend: "Tangguhkan",
    restore: "Pulihkan",
    export: "Ekspor",
    suspendOrg: "Tangguhkan Organisasi",
    suspendOrgDesc: "Yakin ingin menangguhkan {name}? Ini akan menonaktifkan operasi tulis.",
    restoreOrg: "Pulihkan Organisasi",
    restoreOrgDesc: "Yakin ingin memulihkan {name}? Ini akan mengaktifkan kembali operasi tulis.",
    exportOrg: "Ekspor Data Organisasi",
    exportOrgDesc: "Mulai ekspor data untuk {name}?",
    reasonPlaceholder: "Masukkan alasan tindakan ini...",
    cancel: "Batal",
    confirm: "Konfirmasi",
    orgSuspended: "Organisasi ditangguhkan",
    orgRestored: "Organisasi dipulihkan",
    exportInitiated: "Ekspor dimulai",
    plans: "Paket",
    createPlan: "Buat Paket",
    createPlanDesc: "Buat paket langganan baru.",
    planName: "Nama Paket",
    planSlug: "Slug",
    version: "Versi",
    subscribers: "Pelanggan",
    monthlyPrice: "Harga Bulanan",
    annualPrice: "Harga Tahunan",
    description: "Deskripsi",
    monthlyPriceCents: "Harga Bulanan (sen)",
    annualPriceCents: "Harga Tahunan (sen)",
    status: "Status",
    created: "Dibuat",
    active: "Aktif",
    inactive: "Nonaktif",
    subscriptions: "Langganan",
    billingCadence: "Siklus",
    trialEnds: "Akhir Uji Coba",
    periodEnds: "Akhir Periode",
    auditLog: "Log Audit",
    allActions: "Semua Tindakan",
    timestamp: "Waktu",
    actor: "Pelaku",
    action: "Tindakan",
    organization: "Organisasi",
    reason: "Alasan",
    previous: "Sebelumnya",
    next: "Berikutnya",
    page: "Halaman {page}",
    migrations: "Migrasi",
    reviewedBy: "Ditinjau Oleh",
    reviewedAt: "Ditinjau Pada",
    failureReason: "Alasan Kegagalan",
    platformAdmins: "Admin Platform",
    grantAdmin: "Beri Akses Admin",
    grantAdminDesc: "Beri akses admin platform ke pengguna melalui email.",
    userName: "Nama",
    email: "Email",
    grantedAt: "Diberikan Pada",
    revoke: "Cabut",
    revoked: "Dicabut",
    revokeAdmin: "Cabut Akses Admin",
    revokeAdminDesc: "Yakin ingin mencabut akses admin platform?",
    grant: "Beri",
    adminGranted: "Akses admin diberikan",
    adminRevoked: "Akses admin dicabut",
    emailPlaceholder: "user@example.com",
    create: "Buat",
    planCreated: "Paket dibuat",
    noData: "Tidak ada data tersedia",
    noOrgs: "Tidak ada organisasi ditemukan",
    noPlans: "Tidak ada paket ditemukan",
    noSubscriptions: "Tidak ada langganan ditemukan",
    noAuditEvents: "Tidak ada audit ditemukan",
    noMigrations: "Tidak ada migrasi ditemukan",
    noAdmins: "Tidak ada admin platform ditemukan",
    needsAttention: "Perlu perhatian",
    none: "Tidak ada",
    billingEvents: "Riwayat Tagihan",
    billingEventType: "Tipe Kejadian",
    exports: "Ekspor Data",
    exportStatus: "Status",
    exportFormat: "Format",
    exportSize: "Ukuran",
    downloadExport: "Unduh Ekspor",
    exportData: "Ekspor Data",
    exportDataDesc: "Unduh semua data organisasi sebagai file JSON",
    executeExport: "Ekspor Sekarang",
    exportCompleted: "Ekspor selesai. Anda dapat mengunduhnya dari halaman Ekspor.",
    retentionPolicies: "Kebijakan Retensi",
    createPolicy: "Buat Kebijakan",
    createPolicyDesc: "Atur kebijakan retensi data untuk tabel",
    targetTable: "Tabel Target",
    retentionDays: "Retensi (hari)",
    enabled: "Aktif",
    disabled: "Nonaktif",
    deletePolicy: "Hapus Kebijakan",
    deletePolicyDesc: "Anda yakin ingin menghapus kebijakan retensi ini?",
    policyCreated: "Kebijakan retensi dibuat",
    policyUpdated: "Kebijakan retensi diperbarui",
    policyDeleted: "Kebijakan retensi dihapus",
    subscriptionLimit: "Batas Langganan",
    subscriptionPlan: "Paket",
    usage: "Penggunaan",
    of: "dari",
    used: "digunakan",
    approachingLimit: "Mendekati batas",
    atLimit: "Mencapai batas",
    upgradePrompt: "Tingkatkan paket Anda untuk menambah batas",
    goToSettings: "Ke Pengaturan",
  },
  app: {
    title: "AKZ Production",
    retry: "Coba Lagi",
    language: "Bahasa",
    english: "Inggris",
    indonesian: "Indonesia",
  },
  assetUpload: {
    dropzone: {
      title: "Seret file ke sini",
      hint: "atau klik untuk memilih",
    },
    actions: {
      browse: "Pilih file",
      remove: "Hapus",
      retry: "Coba lagi",
      undo: "Batalkan",
    },
    states: {
      uploaded: "Tersimpan",
      uploading: "Mengunggah...",
      processing: "Memproses...",
      failed: "Gagal",
      done: "Selesai",
    },
    errors: {
      tooLarge: "File terlalu besar",
      wrongType: "Jenis file tidak didukung",
      uploadFailed: "Unggah gagal",
      limitReached: "Jumlah file maksimum tercapai",
      serverError: "Kesalahan server",
    },
    hints: {
      acceptedFormats: "JPG, PNG, PDF, Corel hingga {size}",
      maxSize: "Maks {size}",
    },
  },
  common: {
    back: "Kembali",
    loading: "Memuat",
    close: "Tutup",
    pageNotFound: "Halaman tidak ditemukan",
    pageNotFoundDesc:
      "Halaman yang Anda cari tidak ada atau mungkin telah dipindahkan.",
    goHome: "Ke beranda",
    confirm: "Konfirmasi",
    cancel: "Batal",
    preview: "Pratinjau",
    actions: "Aksi",
    pcs: "pcs",
    type: "Tipe",
    name: "Nama",
    phone: "Telepon",
    workspaces: "Ruang Kerja",
    status: "Status",
    optional: "Opsional",
    required: "Wajib",
    download: "Unduh",
    file: "File",
    noAttachments: "Tidak ada lampiran",
    moreFiles: "+{count} file lagi",
  },
  dataTable: {
    clearFilters: "Hapus filter",
    columnVisibility: "Kolom",
    errorRetry: "Coba lagi",
    errorTitle: "Terjadi kesalahan",
    firstPage: "Halaman pertama",
    lastPage: "Halaman terakhir",
    nextPage: "Halaman berikutnya",
    of: "dari",
    page: "Halaman",
    perPage: "Per halaman",
    previousPage: "Halaman sebelumnya",
    resetColumns: "Atur ulang kolom",
    rowsSelected: "{selected} dari {total} dipilih",
    visibleRows: "{from}-{to} dari {total}",
    loading: "Memuat",
    filterAll: "Semua",
    filters: "Filter",
    applyFilters: "Terapkan",
    cancelFilters: "Batal",
    activeFilters: "Filter aktif",
  },
  orders: {
    title: "Pesanan",
    listDescription: "Kelola dan lacak semua pesanan pelanggan",
    createOrder: "Buat Pesanan",
    editOrder: "Edit Pesanan",
    viewOrder: "Lihat Pesanan",
    customer: "Pelanggan",
    status: "Status",
    total: "Total",
    orderNumber: "Nomor Pesanan",
    validUntil: "Berlaku Sampai",
    notes: "Catatan",
    lineItemNotes: "Catatan Item",
    productName: "Produk",
    designName: "Nama Desain",
    designNamePlaceholder: "contoh: ONIC",
    quantity: "Kuantitas",
    unitPrice: "Harga Satuan",
    totalLabel: "Total",
    lineItems: "Item Pesanan",
    summary: "Ringkasan Pesanan",
    attachments: "Lampiran",
    save: "Simpan",
    saving: "Menyimpan...",
    orderCreated: "Pesanan berhasil dibuat",
    orderUpdated: "Pesanan berhasil diperbarui",
    searchPlaceholder: "Cari pesanan...",
    noOrders: "Belum ada pesanan",
    noOrdersDesc: "Buat pesanan pertama Anda untuk memulai.",
    noResults: "Tidak ada pesanan yang cocok",
    addLineItem: "Tambah Item",
    removeLineItem: "Hapus",
    createCustomer: "Buat Pelanggan",
    customerNamePlaceholder: "Masukkan nama pelanggan",
    customerPhonePlaceholder: "Masukkan nomor telepon",
    addItem: "Tambah Item",
    specification: "Spesifikasi",
    selectProduct: "Pilih Produk",
    orderTotal: "Total Pesanan",
    searchProducts: "Cari produk...",
    addToOrder: "Tambah",
    lineSubtotal: "Subtotal",
    maxQtyError: "Kuantitas maksimal {max}",
    minQtyError: "Kuantitas minimal {min}",
    phone: "Telepon",
    email: "Email",
    copyOrderLink: "Salin Tautan",
    orderLinkCopied: "Tautan pesanan disalin",
    copyPortalLink: "Salin Tautan Portal",
    linkCopied: "Tautan portal disalin",
    generateLink: "Buat Tautan Portal",
    generateLinkFailed: "Gagal membuat tautan",
    approve: "Setujui",
    reject: "Tolak",
    rejectReason: "Alasan Penolakan",
    rejectReasonPlaceholder: "Masukkan alasan penolakan",
    orderApproved: "Pesanan disetujui",
    orderRejected: "Pesanan ditolak",
    orderCompleted: "Pesanan selesai",
    statusDraft: "Draf",
    statusPending: "Tertunda",
    statusApproved: "Disetujui",
    statusInProgress: "Sedang Diproses",
    statusProduction: "Dalam Produksi",
    statusInDelivery: "Dalam Pengiriman",
    statusCompleted: "Selesai",
    statusCancelled: "Dibatalkan",
    statusRejected: "Ditolak",
    completeOrder: "Selesaikan Pesanan",
    guestCustomer: "Tamu",
    paymentStatus: "Status Pembayaran",
    dueDate: "Jatuh Tempo",
    createdAt: "Dibuat Pada",
    paymentNoInvoice: "Tanpa Invoice",
    paymentPaid: "Lunas",
    paymentUnpaid: "Belum Dibayar",
    paymentPartiallyPaid: "Dibayar Sebagian",
    paymentVoid: "Batal",
    deadline: "Deadline",
    downloadQuotation: "Unduh Penawaran",
    sendDpInvoice: "Kirim Invoice DP",
    confirmDpPayment: "Konfirmasi DP",
    sendSettlementInvoice: "Kirim Invoice Pelunasan",
    confirmSettlementPayment: "Konfirmasi Pelunasan",
    loadOrdersFailed: "Tidak dapat memuat pesanan",
    loadOrdersFailedDesc: "Periksa koneksi Anda lalu coba lagi.",
    copyLinkFailed: "Tidak dapat menyalin tautan portal",
    openPortalLink: "Buka Tautan Portal",
    customerDueDate: "Jatuh Tempo Pelanggan",
    shippingDetail: "Detail Pengiriman",
    internalDeadline: "Tenggat Produksi",
    overdueDeadline: "Terlambat",
    completedAt: "Selesai Pada",
    hargaNego: "Harga Negosiasi",
    repeatOrder: "Repeat Order",
    addons: "Addon",
    manualDeadline: "Tenggat Manual",
    manualDeadlineRequired:
      "Tenggat manual diperlukan untuk kuantitas yang melebihi kapasitas produksi",
    quantityExceedsProductionCap:
      "Kuantitas melebihi kapasitas produksi. Tenggat manual diperlukan.",
    createOrderFailed:
      "Tidak dapat membuat pesanan. Periksa setup lalu coba lagi.",
    setupMissionLocked: "Setup diperlukan",
    setupMissionTitle: "Selesaikan setup sebelum membuat pesanan",
    setupMissionDescription:
      "Pesanan membutuhkan alamat bisnis, alur produksi, katalog produk, dan instruksi pembayaran. Lengkapi sekali, lalu pembuatan pesanan akan terbuka.",
    setupMissionProgress: "{completed} dari {total} siap",
    setupMissionListLabel: "Checklist setup pesanan",
    setupMissionComplete: "Siap",
    setupMissionIncomplete: "Perlu",
    setupBusinessAddressTitle: "Atur alamat bisnis",
    setupBusinessAddressDesc:
      "Dipakai pada penawaran, invoice, dan detail pengiriman.",
    setupBusinessAddressAction: "Atur alamat",
    setupProductionStagesTitle: "Buat tahapan produksi",
    setupProductionStagesDesc:
      "Tambahkan minimal satu tahapan Produksi aktif agar pesanan yang disetujui bisa masuk ke lantai produksi.",
    setupProductionStagesAction: "Atur tahapan",
    setupFirstProductTitle: "Buat produk pertama",
    setupFirstProductDesc:
      "Pesanan membutuhkan produk aktif dengan harga dan hari produksi.",
    setupFirstProductAction: "Buat produk",
    setupPaymentMethodTitle: "Tambah metode pembayaran",
    setupPaymentMethodDesc:
      "Invoice membutuhkan rekening atau instruksi pembayaran agar pelanggan bisa membayar.",
    setupPaymentMethodAction: "Tambah metode",
    setupMissionHelpTitle: "Mengapa ini dikunci",
    setupMissionHelpBody:
      "Membuat pesanan terlalu awal meninggalkan data kosong yang mengganggu proses berikutnya.",
    setupMissionHelpAddress:
      "Data alamat membuat penawaran, invoice, dan catatan pengiriman tetap konsisten.",
    setupMissionHelpProduction:
      "Tahapan produksi menentukan tempat kerja yang disetujui masuk di papan produksi.",
    setupMissionHelpPayment:
      "Metode pembayaran membuat invoice langsung bisa ditindaklanjuti pelanggan sejak pesanan pertama.",
    orderDeadline: "Deadline pesanan",
    manualOrderDeadline: "Atur deadline pesanan manual",
    manualOrderDeadlineDescription:
      "Aktifkan untuk mengatur deadline custom untuk seluruh pesanan, bukan per item.",
    orderDeadlineAutoDescription:
      "Jika nonaktif, deadline pesanan mengikuti deadline item paling akhir.",
    orderDeadlineRequired: "Deadline pesanan wajib diisi",
    adjustQuantity: "Sesuaikan Jumlah",
    adjustQuantityDescription:
      "Ubah jumlah item pesanan. Total pesanan dan harga akan dihitung ulang.",
    selectLineItem: "Pilih Item Pesanan",
    currentQuantity: "Jumlah Saat Ini",
    newQuantity: "Jumlah Baru",
    adjustmentReason: "Alasan",
    adjustmentReasonPlaceholder: "Masukkan alasan penyesuaian ini",
    quantityAdjusted: "Jumlah berhasil disesuaikan",
  },
  breadcrumb: {
    dashboard: "Dasbor",
    detail: "Detail",
    edit: "Edit",
    new: "Baru",
    customers: "Pelanggan",
    createCustomer: "Buat Pelanggan",
    editCustomer: "Edit Pelanggan",
    viewCustomer: "Lihat Pelanggan",
    products: "Produk",
    newProduct: "Produk Baru",
    editProduct: "Edit Produk",
    orders: "Pesanan",
    createOrder: "Buat Pesanan",
    viewOrder: "Lihat Pesanan",
    editOrder: "Edit Pesanan",
    invoices: "Invoice",
    createInvoice: "Buat Invoice",
    viewInvoice: "Lihat Invoice",
    settings: "Pengaturan",
    general: "Perusahaan",
    production: "Produksi",
    productionStages: "Tahapan Produksi",
    paymentMethods: "Metode Pembayaran",
    members: "Anggota",
    profile: "Profil",
    signIn: "Masuk",
    signUp: "Buat akun",
    createOrganization: "Buat Perusahaan",
    accessDenied: "Akses ditolak",
    noOrganization: "Belum tergabung perusahaan",
    completeOrderData: "Lengkapi Data Pesanan",
    acceptInvitation: "Terima Undangan",
    notifications: "Notifikasi",
    channels: "Saluran",
  },
  status: {
    draft: "Draf",
    pending: "Tertunda",
    approved: "Disetujui",
    in_progress: "Sedang Diproses",
    production: "Dalam Produksi",
    in_delivery: "Dalam Pengiriman",
    completed: "Selesai",
    readyToShip: "Selesai",
    cancelled: "Dibatalkan",
    rejected: "Ditolak",
    active: "Aktif",
    inactive: "Tidak Aktif",
    paid: "Lunas",
    partially_paid: "Dibayar Sebagian",
    unpaid: "Belum Dibayar",
    void: "Batal",
    overdue: "Jatuh Tempo",
    pendingPayment: "Menunggu Konfirmasi",
    failed: "Gagal",
    confirmed: "Dikonfirmasi",
    refunded: "Dikembalikan",
    queued: "Dalam Antrian",
    pending_approval: "Menunggu Persetujuan",
    accepted: "Diterima",
    canceled: "Dibatalkan",
    deleted: "Dihapus",
  },
  org: {
    title: "Perusahaan",
    welcome: "Selamat datang di Pabriq",
    createDesc: "Beri nama perusahaan Anda untuk memulai",
    create: "Buat Perusahaan",
    name: "Nama Perusahaan",
    namePlaceholder: "Cth. Bengkel Saya",
    nameMin: "Nama harus minimal 2 karakter",
    nameInvalid: "Nama perusahaan tidak valid",
    creating: "Membuat...",
    creationFailed: "Gagal membuat perusahaan",
    provisioningFailed: "Gagal menginisialisasi penyiapan organisasi",
    taken: "Nama tersebut sudah digunakan. Silakan coba nama perusahaan lain.",
    redirecting: "Mengarahkan...",
    logoPhoto: "Foto Perusahaan",
    logoPhotoHint: "PNG, JPG, atau WebP hingga 5MB",
    clickToUpload: "Klik untuk unggah foto",
  },
  sidebar: {
    dashboard: "Dasbor",
    orders: "Pesanan",
    customers: "Pelanggan",
    products: "Produk",
    invoices: "Invoice",
    production: "Produksi",
    settings: "Pengaturan",
    admin: "Admin",
    notifications: "Notifikasi",
  },
  settings: {
    general: "Perusahaan",
    members: "Anggota",
    profile: "Profil",
    paymentMethods: "Metode Pembayaran",
    invoicing: "Invoice",
    orgName: "Nama Perusahaan",
    orgSlug: "Slug",
    phone: "Nomor Telepon",
    email: "Email",
    address: "Alamat",
    billingAddress: "Alamat Penagihan",
    logo: "Logo Perusahaan",
    saved: "Pengaturan disimpan",
    saveFailed: "Gagal menyimpan pengaturan",
    profileTab: "Profil",
    memberTab: "Anggota",
    addPaymentMethod: "Tambah Metode Pembayaran",
    editPaymentMethod: "Edit Metode Pembayaran",
    bankTransfer: "Transfer Bank",
    paymentGateway: "Payment Gateway",
    midtransIntegration: "Integrasi Midtrans",
    bankAccounts: "Rekening Bank",
    midtransServerKey: "Server Key Midtrans",
    midtransClientKey: "Client Key Midtrans",
    midtransProduction: "Gunakan Midtrans production",
    midtransIntegrationDescription:
      "Atur satu payment gateway Midtrans untuk pembayaran invoice online.",
    midtransCredentialsRequired: "Masukkan server key dan client key Midtrans.",
    midtransWebhookUrl: "URL Webhook",
    bankName: "Nama Bank",
    accountNumber: "Nomor Rekening",
    accountHolder: "Nama Pemilik Rekening",
    instructions: "Instruksi",
    defaultPayment: "Default",
    deletePaymentMethod: "Hapus Metode Pembayaran",
    deletePaymentMethodConfirm:
      "Apakah Anda yakin ingin menghapus metode pembayaran ini?",
    noPaymentMethods: "Belum ada metode pembayaran",
    noPaymentMethodsDesc: "Tambahkan metode pembayaran untuk memulai.",
    active: "Aktif",
    inactive: "Tidak Aktif",
    organization: "Perusahaan",
    lateFeePerDay: "Cashback terlambat per hari",
    lateFeePerDayDescription:
      "Potong nominal ini dari invoice pelunasan untuk setiap hari kalender setelah deadline pesanan. Isi 0 untuk menonaktifkan.",
    midtransWebhookCopied: "Disalin",
    configured: "Terkonfigurasi",
    notConfigured: "Belum dikonfigurasi",
    save: "Simpan",
    channels: "Saluran",
  },
  members: {
    title: "Operator",
    invite: "Tambah Operator",
    inviteDesc: "Undang admin atau buat akun operator untuk perusahaan Anda",
    name: "Nama",
    email: "Email",
    role: "Peran",
    ownerRole: "Pemilik",
    adminRole: "Admin",
    memberRole: "Operator",
    operatorRole: "Operator (portal)",
    joined: "Bergabung",
    remove: "Hapus",
    removeConfirm: "Yakin ingin menghapus operator ini?",
    changeRole: "Ubah Peran",
    pending: "Undangan Tertunda",
    pendingDesc: "Undangan ini belum diterima",
    cancelInvite: "Batalkan",
    cancelConfirm: "Yakin ingin membatalkan undangan ini?",
    inviteSent: "Undangan terkirim",
    inviteLinkCopied: "Tautan undangan disalin",
    acceptPageTitle: "Terima Undangan",
    acceptDesc: "Anda telah diundang untuk bergabung",
    accept: "Terima",
    reject: "Tolak",
    accepted: "Undangan diterima",
    rejected: "Undangan ditolak",
    roleUpdated: "Peran diperbarui",
    roleUpdateFailed: "Gagal memperbarui peran",
    memberRemoved: "Operator dihapus",
    copyLink: "Salin Tautan Undangan",
    noMembers: "Belum ada operator",
    noMembersDesc: "Buat akun operator untuk perusahaan Anda untuk memulai",
    defaultPassword: "Kata sandi default",
    operatorAccountCreated:
      "Akun operator dibuat. Masuk dengan {email} dan kata sandi {password}.",
    operatorCredentialsCopied: "Kredensial operator disalin",
  },
  profile: {
    title: "Profil",
    name: "Nama",
    email: "Email",
    avatar: "Avatar",
    changePassword: "Ubah Kata Sandi",
    currentPassword: "Kata Sandi Saat Ini",
    newPassword: "Kata Sandi Baru",
    newPasswordDesc: "Minimal 8 karakter",
    passwordChanged: "Kata sandi berhasil diubah",
    passwordChangeFailed: "Gagal mengubah kata sandi",
    profileSaved: "Profil disimpan",
    profileSaveFailed: "Gagal menyimpan profil",
  },
  customers: {
    title: "Pelanggan",
    listDescription: "Pantau pelanggan dan detail mereka",
    createCustomer: "Buat Pelanggan",
    editCustomer: "Edit Pelanggan",
    viewCustomer: "Lihat Pelanggan",
    customerInfo: "Informasi Pelanggan",
    name: "Nama",
    email: "Email",
    phone: "Telepon",
    notes: "Catatan",
    active: "Aktif",
    inactive: "Tidak Aktif",
    searchPlaceholder: "Cari pelanggan...",
    noCustomers: "Belum ada pelanggan",
    noCustomersDesc:
      "Tambah pelanggan pertama Anda untuk mulai mengelola pesanan.",
    noResults: "Tidak ada pelanggan yang cocok",
    save: "Simpan",
    saving: "Menyimpan...",
    delete: "Hapus",
    deleted: "Pelanggan dihapus",
    deleteConfirm: "Apakah Anda yakin ingin menghapus pelanggan ini?",
    customerCreated: "Pelanggan berhasil dibuat",
    customerUpdated: "Pelanggan berhasil diperbarui",
    nameRequired: "Nama harus diisi",
    photo: "Foto",
    uploadPhoto: "Unggah Foto",
    removePhoto: "Hapus Foto",
    createdAt: "Dibuat Pada",
    recentOrders: "Pesanan Terbaru",
    recentInvoices: "Tagihan",
    financialSummary: "Ringkasan Keuangan",
    totalOrders: "Total Pesanan",
    totalSpent: "Total Belanja",
    outstandingBalance: "Saldo Tertunggak",
    citizenship: "Kewarganegaraan",
    address: "Alamat",
    noShippingAddress: "Belum ada alamat pengiriman",
  },
  dashboard: {
    title: "Dasbor",
    description:
      "Pantau pendapatan, alur tugas, dan aktivitas terbaru secara ringkas.",
    welcome: "Selamat datang di ruang kerja Pabriq Anda",
    activeOrders: "Pesanan Aktif",
    activeOrdersDesc: "Pesanan yang sedang diproduksi",
    products: "Produk",
    productsDesc: "Produk di katalog Anda",
    invoices: "Invoice",
    invoicesDesc: "Invoice yang belum dibayar",
    orderFallback: "Pesanan draft",
    emptyRevenue: "Belum ada data pendapatan untuk periode ini",
    emptyTaskStages: "Belum ada tugas produksi",
    noRecentOrders: "Belum ada pesanan terbaru",
    taskCountSuffix: "tugas",
    trend: {
      up: "Naik",
      down: "Turun",
      noChange: "Tidak berubah",
      live: "Live",
      vsPreviousPeriod: "vs periode sebelumnya",
    },
    kpi: {
      revenue: "Pendapatan",
      revenueDesc: "Pendapatan dari pesanan selesai pada periode terpilih",
      orders: "Pesanan",
      ordersDesc: "Jumlah pesanan yang dibuat pada periode terpilih",
      overdueInvoices: "Invoice Jatuh Tempo",
      overdueInvoicesDesc: "Invoice yang sudah melewati tanggal jatuh tempo",
      pendingApprovals: "Menunggu Persetujuan",
      pendingApprovalsDesc: "Pesanan yang sedang menunggu persetujuan admin",
    },
    period: {
      "7d": "7 Hari",
      "30d": "30 Hari",
      thisMonth: "Bulan Ini",
      lastMonth: "Bulan Lalu",
    },
    sections: {
      revenue: "Total Pendapatan",
      revenueDesc: "Tren pendapatan harian dari pesanan selesai",
      recentOrders: "Pesanan Terbaru",
      recentOrdersDesc: "Update pesanan terbaru dari workspace Anda",
      taskStages: "Tugas per Tahap",
      taskStagesDesc: "Beban kerja produksi saat ini di tiap tahap",
    },
    viewAll: "Lihat Semua",
  },
  products: {
    title: "Produk",
    listDescription: "Kelola katalog produk dan harga Anda",
    createTitle: "Produk Baru",
    editTitle: "Edit Produk",
    productInfo: "Informasi Produk",
    pricingAndOrders: "Harga & Pesanan",
    name: "Nama Produk",
    namePlaceholder: "Cth. Kaos Kustom",
    description: "Deskripsi",
    descriptionPlaceholder: "Jelaskan produk",
    productionNotes: "Catatan Produksi",
    productionNotesPlaceholder: "Instruksi khusus untuk produksi",
    priority: "Produk prioritas",
    priorityDescription:
      "Tugas yang dibuat dari produk ini akan ditandai sebagai prioritas.",
    active: "Aktif",
    inactive: "Tidak Aktif",
    searchPlaceholder: "Cari produk...",
    noProducts: "Belum ada produk",
    noProductsDesc: "Buat produk pertama Anda untuk memulai katalog.",
    noResults: "Tidak ada produk yang cocok",
    createProduct: "Buat Produk",
    updateProduct: "Perbarui Produk",
    created: "Produk berhasil dibuat",
    updated: "Produk berhasil diperbarui",
    deleted: "Produk dihapus",
    deleteProduct: "Hapus Produk",
    deleteConfirm: "Anda yakin ingin menghapus produk ini?",
    createdAt: "Dibuat Pada",
    price: "Harga",
    basePrice: "Harga Dasar",
    productionDays: "Hari Produksi",
    minQuantity: "Min. Kuantitas",
    maxQuantity: "Maks. Kuantitas",
    noPhoto: "Tanpa foto",
    photo: "Foto Utama",
    viewProduct: "Lihat Produk",
    editProduct: "Edit Produk",
    advancedSettings: "Harga & Konfigurasi Lanjutan",
    advancedSettingsDescription:
      "Konfigurasikan Repeat Order, tingkatan harga volume, dan addon",
    basePricingLimits: "Harga Dasar & Batas Kuantitas",
    pricing: {
      title: "Harga",
      breakpoints: "Tingkatan Harga",
      unitPrice: "Harga Satuan",
      minQuantity: "Min. Kuantitas",
      addBreakpoint: "Tambah Tingkatan",
      noBreakpoints: "Belum ada tingkatan harga",
      preview: "Pratinjau Harga",
      interpolate: "Interpolasi harga",
      interpolateOn:
        "Harga dihitung menggunakan interpolasi linier antar tingkatan",
      interpolateOff:
        "Harga menggunakan tingkatan terendah terdekat (step pricing)",
    },
    negotiateAboveQuantity: "Negosiasi Di Atas Kuantitas",
    negotiateAboveQuantityDescription:
      "Pesanan yang melebihi kuantitas ini akan memicu negosiasi manual, bukan harga satuan standar.",
    repeatOrderUnitPrice: "Harga Satuan Repeat Order",
    repeatOrderUnitPriceDescription:
      "Harga satuan khusus untuk pesanan berulang dari pelanggan.",
    repeatOrderMinQuantity: "Min. Kuantitas Repeat Order",
    repeatOrderMinQuantityDescription:
      "Kuantitas minimum untuk mendapatkan harga pesanan berulang.",
    maxProductionQuantity: "Maks. Kuantitas Produksi",
    maxProductionQuantityDescription:
      "Pesanan yang melebihi kuantitas ini tetap dapat diterima, tetapi waktu produksi mungkin lebih lama dari estimasi hari standar.",
    addons: {
      title: "Addon",
      addAddon: "Tambah Addon",
      noAddons: "Belum ada addon",
      name: "Nama Addon",
      unitSurcharge: "Biaya Tambahan Satuan",
    },
  },
  invoices: {
    title: "Invoice",
    listDescription: "Lihat dan kelola semua invoice dan pembayaran",
    loadInvoicesFailed: "Gagal memuat invoice",
    loadInvoicesFailedDesc: "Periksa koneksi Anda dan coba lagi.",
    createInvoice: "Buat Invoice",
    viewInvoice: "Lihat Invoice",
    invoiceNumber: "No. Invoice",
    customer: "Pelanggan",
    total: "Total",
    dueDate: "Jatuh Tempo",
    issuedDate: "Diterbitkan",
    status: "Status",
    percentage: "Persentase",
    subtotal: "Subtotal",
    lineItems: "Item",
    notes: "Catatan",
    markAsPaid: "Tandai Lunas",
    voidInvoice: "Batalkan Invoice",
    voidConfirm: "Anda yakin ingin membatalkan Invoice ini?",
    paymentMethod: "Metode Pembayaran",
    description: "Deskripsi",
    qty: "Jml",
    rate: "Harga",
    addItem: "Tambah Item",
    amount: "Jumlah",
    method: "Metode",
    reference: "Referensi",
    payments: "Pembayaran",
    paymentHistory: "Riwayat Pembayaran",
    paidAmount: "Dibayar",
    pendingAmount: "Menunggu",
    remaining: "Sisa",
    recordPayment: "Catat Pembayaran",
    paymentSource: "Sumber Pembayaran",
    gatewayOrderId: "ID Pesanan Gateway",
    gatewayTransactionId: "ID Transaksi Gateway",
    manualReviewRequired: "Perlu peninjauan manual",
    reconcileFailed: "Pengecekan rekonsiliasi gagal",
    transactionHistory: "Riwayat transaksi Midtrans ({count})",
    transactionError: "Kesalahan gateway",
    midtransOnlineGateway: "Midtrans (Gateway Online)",
    bankTransferManual: "Transfer Bank (Manual)",
    transactionId: "ID Transaksi: {id}",
    paymentRecords: "Catatan Pembayaran ({count})",
    referencePrefix: "Ref: {reference}",
    confirmSimple: "Konfirmasi",
    rejectSimple: "Tolak",
    rejectReasonPlaceholder: "Alasan penolakan",
    rejectPayment: "Tolak Pembayaran",
    rejectPaymentDesc: "Menolak pembayaran akan menandai pembayaran sebagai ditolak. Harap sertakan alasan yang jelas untuk audit.",
    rejectionReason: "Alasan Penolakan",
    rejectionReasonRequired: "Alasan wajib diisi saat menolak pembayaran",
    confirmRejection: "Konfirmasi Penolakan",
    confirmedByLabel: "Dikonfirmasi oleh",
    receivedAtLabel: "Diterima pada",
    syncTimeLabel: "Sinkronisasi terakhir",
    rejectedReasonLabel: "Alasan penolakan",
    viewProof: "Lihat Bukti",
    paymentProof: "Bukti Pembayaran",
    uploadProof: "Upload Bukti Pembayaran",
    paymentConfirmed: "Pembayaran dikonfirmasi",
    paymentRecorded: "Pembayaran dicatat",
    paymentRejected: "Pembayaran ditolak",
    invalidAmount: "Jumlah tidak valid",
    bankTransfer: "Transfer Bank",
    midtrans: "Midtrans",
    paymentCash: "Tunai",
    confirmPayment: "Konfirmasi Pembayaran",
    pendingConfirmation: "Menunggu konfirmasi",
    downloadInvoice: "Unduh Invoice",
    downloadProof: "Unduh Bukti",
    printInvoice: "Cetak Invoice",
    invoiceCreated: "Invoice dibuat",
    paymentProofUploaded: "Bukti pembayaran diunggah",
    invoicePaid: "Invoice lunas",
    invoiceVoided: "Invoice dibatalkan",
    reconcileWithMidtrans: "Cek transaksi Midtrans",
    reconcilePaid: "Pembayaran dikonfirmasi via Midtrans",
    reconcileNotSettled: "Midtrans melaporkan transaksi ini belum selesai",
    reconcileNoOrderId: "Tidak ada transaksi Midtrans terkait invoice ini",
    reconcileMismatch: "Nominal Midtrans tidak cocok dengan total invoice",
    reconcileGatewayUnavailable: "Midtrans sedang tidak tersedia. Silakan coba lagi nanti.",
    reconcileManualReview: "Pembayaran ini perlu ditinjau manual oleh operator.",
    viewPaymentProof: "Lihat bukti pembayaran",
    overdue: "Jatuh Tempo",
    noInvoices: "Belum ada Invoice",
    noInvoicesDesc: "Buat Invoice pertama Anda untuk memulai.",
    searchPlaceholder: "Cari Invoice...",
    noResults: "Tidak ada Invoice yang cocok",
    createdAt: "Dibuat Pada",
    uploadFailed: "Bukti pembayaran gagal diunggah. Coba lagi.",
    proofRemoved: "Bukti pembayaran berhasil dihapus",
    removeFailed: "Gagal menghapus bukti pembayaran",
    submitPaymentProof: "Kirim Bukti Pembayaran",
    resendPaymentProof: "Kirim Ulang Bukti Pembayaran",
    paymentNote: "Catatan transfer / referensi",
    paymentNotePlaceholder: "Mis. Transfer BCA jam 11:30",
    submitProofSuccess: "Bukti pembayaran terkirim. Menunggu konfirmasi admin.",
    submitProofFailed: "Gagal mengirim bukti pembayaran. Coba lagi.",
    uploadFilesFirst: "Harap unggah minimal satu file bukti terlebih dahulu.",
    failed: "Gagal",
    orderLabel: "Pesanan #{orderNumber}",
    alreadyInvoiced: "Sudah ditagih: {percentage}% ({amount})",
    invoiceAmount: "Jumlah invoice",
    fullAmount: "Penuh (100%)",
    customAmount: "Kustom",
    remainingAmount: "Sisa ({percentage}%)",
    stepHint: "(kelipatan {percentage}%)",
    decreasePercentage: "Kurangi persentase",
    increasePercentage: "Tambah persentase",
    invoiceTotal: "Total invoice: {amount}",
    remainingFrom: "(sisa dari {amount})",
    customerName: "Nama Pelanggan",
    transferFallback: "Transfer",
    confirmManualShipmentFee: "Konfirmasi biaya pengiriman manual",
    shipmentMethod: "Metode Pengiriman",
    shipmentMethodBiteship: "Hitung dengan Biteship",
    shipmentMethodManual: "Manual / Luar Biteship",
    shipmentMethodPickup: "Ambil Sendiri / Tanpa Pengiriman",
    packageWeightKg: "Berat Paket (kg)",
    calculateShipmentFee: "Hitung Biaya Pengiriman",
    shipmentRate: "Tarif Pengiriman",
    selectShipmentRateRequired: "Pilih tarif pengiriman sebelum mengirim",
    noShipmentRatesFound: "Tidak ada tarif pengiriman yang ditemukan",
    biteshipApiKeyMissing: "API key Biteship belum dikonfigurasi",
    biteshipRateCalculationFailed: "Gagal menghitung tarif pengiriman",
    shippingAreaRequired: "Area pengiriman wajib diisi",
    packageWeightRequired: "Berat paket wajib diisi",
    customerPickupNoShipping:
      "Pelanggan akan mengambil pesanan. Tidak ada biaya pengiriman.",
    lateFeeCashback: "Cashback keterlambatan",
    lateFeeCashbackDescription: "{days} hari terlambat × {amount} per hari",
    confirmManualPayment: "Konfirmasi Pembayaran Manual",
    confirmManualPaymentDesc:
      "Tinjau bukti pembayaran dan konfirmasi transfer bank ini.",
    destinationBank: "Bank Tujuan",
    bankName: "Nama Bank",
    accountNumber: "Nomor Rekening",
    accountHolder: "Nama Pemilik Rekening",
    noPaymentMethodConfigured:
      "Belum ada rekening bank yang dikonfigurasi untuk invoice ini.",
    latestProofSubmitted: "Bukti terakhir dikirim",
    paymentProofs: "Bukti Pembayaran",
    noPaymentProofs: "Belum ada bukti pembayaran yang dikirim.",
  },
  address: {
    title: "Alamat",
    areaSearch: "Cari area...",
    areaSearchPlaceholder: "Cari kecamatan, kota, atau kode pos",
    startTypingToSearch: "Mulai ketik untuk mencari area",
    searchingAreas: "Mencari area...",
    noResults: "Area tidak ditemukan",
    streetAddress: "Alamat Jalan",
    streetAddressPlaceholder: "cth. Jl. Raya Bogor No. 123",
    saveAsCustomerAddress: "Simpan sebagai alamat pelanggan",
    isWni: "Warga Negara Indonesia (WNI)",
    isWna: "Warga Negara Asing (WNA)",
    areaNotSupported:
      "Perhitungan pengiriman tidak tersedia untuk pelanggan WNA",
    orgAddressRequired: "Silakan atur alamat perusahaan Anda untuk melanjutkan",
    areaNotFound: "Area tidak ditemukan. Mohon periksa nama area.",
    defaultAddress: "Alamat default",
  },
  production: {
    title: "Produksi",
    close: "Tutup",
    kanbanTab: "Kanban",
    listTab: "Daftar",
    stagesTab: "Tahapan",
    searchPlaceholder: "Cari pesanan...",
    allStages: "Semua Tahapan",
    queue: "Antrian",
    done: "Selesai",
    noTasks: "Belum ada tugas",
    taskDetail: "Detail Tugas",
    specification: "Spesifikasi",
    materialsRequired: "Bahan yang Dibutuhkan",
    wastePercentage: "(+{percentage}% toleransi)",
    attachments: "Lampiran",
    proofFiles: "File Bukti",
    activity: "Aktivitas",
    comments: "Komentar",
    commentPlaceholder: "Tambah komentar...",
    send: "Kirim",
    startProduction: "Mulai Pra-Produksi",
    continueToProduction: "Lanjut Produksi",
    advanceTo: "Lanjutkan ke {stage}",
    completeRequirements: "Lengkapi Persyaratan",
    requirementRequired: "Wajib",
    requirementOptional: "Opsional",
    uploadFile: "Unggah Berkas",
    requestReview: "Minta Review",
    reviewAdvancement: "Review Lanjutan",
    approve: "Setujui & Lanjutkan",
    approveOrder: "Setujui Pesanan",
    reject: "Tolak",
    rejectOrder: "Tolak Pesanan",
    cancelOrder: "Batalkan Pesanan",
    reviewNotes: "Catatan Review",
    reviewTaskLabel: "Tugas",
    reviewStageLabel: "Tahapan",
    fulfilledRequirements: "Persyaratan Terpenuhi",
    attachmentCount: "{count, plural, one {# berkas} other {# berkas}}",
    openTask: "Buka tugas {task}",
    columnTaskCount: "{column}: {count, plural, one {# tugas} other {# tugas}}",
    canceled: "Dibatalkan",
    stageManagement: "Tahapan Produksi",
    addStage: "Tambah Tahapan",
    editStage: "Edit Tahapan",
    deleteStage: "Hapus Tahapan",
    deleteStageConfirm: "Anda yakin ingin menghapus tahapan ini?",
    stageName: "Nama Tahapan",
    stageDescription: "Deskripsi",
    stageDescriptionPlaceholder: "Jelaskan tahapan ini",
    needApproval: "Perlu Persetujuan",
    needApprovalHint:
      "Melanjutkan melewati tahapan ini memerlukan persetujuan admin",
    active: "Aktif",
    inactive: "Tidak Aktif",
    requirements: "Persyaratan",
    addRequirement: "Tambah Persyaratan",
    requirementLabel: "Label",
    requirementType: "Tipe",
    requirementTypeText: "Teks",
    requirementTypeNumber: "Angka",
    requirementTypeUpload: "Unggah",
    required: "Wajib",
    optional: "Opsional",
    reorder: "Urutkan Ulang",
    movedToStage: "Dipindahkan ke tahap berikutnya",
    advancedFromQueue: "Maju dari Antrian",
    advancementRequested: "Kemajuan diminta",
    approved: "Disetujui oleh {actor}",
    rejected: "Ditolak oleh {actor}",
    savedRequirement: "Persyaratan disimpan",
    taskCompleted: "Tugas siap dikirim",
    orderApproved: "Pesanan berhasil disetujui",
    orderRejected: "Pesanan ditolak",
    pendingApproval: "Menunggu Persetujuan",
    statusQueued: "Dalam Antrian",
    statusInProgress: "Sedang Dikerjakan",
    statusCompleted: "Selesai",
    orderLabel: "Pesanan",
    productLabel: "Produk",
    designName: "Nama Desain",
    customerLabel: "Pelanggan",
    quantityLabel: "Kuantitas",
    priorityBadge: "Prioritas",
    needReview: "Perlu Review",
    deadlineToday: "Jatuh tempo hari ini",
    deadlineTomorrow: "Jatuh tempo besok",
    deadlineDaysLeft: "{days, plural, one {# hari lagi} other {# hari lagi}}",
    deadlineDaysOverdue:
      "{days, plural, one {# hari terlambat} other {# hari terlambat}}",
    deadlineLabel: "Tenggat {date}",
    deadlineFinishedEarly:
      "{days, plural, one {Lebih cepat # hari} other {Lebih cepat # hari}}",
    deadlineOnTime: "Tepat waktu",
    deadlineFinishedLate:
      "{days, plural, one {Lebih lambat # hari} other {Lebih lambat # hari}}",
    taskCreated: "Tugas dibuat",
    activityAdvancementRequested: "Mengajukan kemajuan ke {stage}",
    activityApproved: "Kemajuan disetujui",
    activityRejected: "Kemajuan ditolak",
    noActivity: "Belum ada aktivitas",
    tabActive: "Tugas Aktif",
    tabArchive: "Arsip",
    board: "Papan",
    kanbanTitle: "Kanban",
    boardPreProduction: "Pra-Produksi",
    boardProduction: "Produksi",
    archivedEmpty: "Belum ada tugas yang diarsipkan",
    archivedTaskNumber: "Nomor Tugas",
    archivedOrder: "Pesanan",
    archivedProduct: "Produk",
    archivedCustomer: "Pelanggan",
    archivedDate: "Diarsipkan",
    requirementRemove: "Hapus persyaratan",
    productionTasks: "Tugas Produksi",
    noTasksForOrder: "Tidak ada tugas produksi untuk pesanan ini",
    loading: "Memuat...",
    // Complete production / shipment
    markAsShipped: "Konfirmasi Pengiriman",
    completeProduction: "Selesaikan Produksi",
    shipmentDetails: "Detail Pengiriman",
    shipmentAddress: "Alamat Pengiriman",
    shipmentFee: "Biaya Pengiriman",
    shipmentFeePlaceholder: "Masukkan biaya pengiriman (opsional)",
    courierPlaceholder: "cth: JNE, SiCepat, J&T",
    trackingNumber: "Nomor Resi",
    trackingNumberPlaceholder: "Masukkan nomor resi",
    allTasksCompleted: "Semua tugas telah selesai ✓",
    tasksNotCompleted: "Beberapa tugas masih dalam proses",
    remainingPayment: "Sisa Pembayaran",
    createInvoiceAndShip: "Buat Invoice & Kirim",
    courier: "Kurir",
    payment: "Pembayaran",
    finalInvoice: "Invoice Akhir",
    orderTotal: "Total Pesanan",
    alreadyPaid: "Sudah Dibayar",
    shippingFeeDescription: "Deskripsi",
    shippingFeeDescriptionPlaceholder: "cth: Biaya Pengiriman",
    total: "Total",
    paymentMethodRequired: "Pilih metode pembayaran",
    trackingNumberRequired: "Nomor resi wajib diisi jika menggunakan kurir",
    invoiceAmountRequired: "Jumlah invoice harus lebih dari 0",
    completeProductionFailed: "Gagal",
    orderNumberLabel: "Pesanan #",
    startOrderProduction: "Mulai Produksi",
    productionStarted: "Produksi dimulai",
    startProductionFailed: "Gagal memulai produksi",
    readyForProduction: "Pembayaran DP",
    readyForProductionQueue: "Antrian {stage}",
    markReadyForProduction: "Tandai Pembayaran DP",
    tabThreeColumn: "Tampilan tiga kolom",
    queueReady: "Siap dikerjakan",
    queueWaiting: "Menunggu",
    noReadyTasks: "Tidak ada tugas yang siap dikerjakan",
    noWaitingTasks: "Tidak ada tugas yang menunggu persetujuan",
    stagePending: "Dalam antrian",
    workflow: "Alur kerja",
    // Workflow step subtitles
    stepSubtitleReleased: "Dirilis",
    stepSubtitleInQueue: "Dalam antrian",
    stepSubtitleReadyToShip: "Siap dikirim",
    stepSubtitlePendingReview: "Menunggu review",
    stepSubtitleCurrentStage: "Tahap saat ini",
    stepSubtitleNextStage: "Tahap berikutnya",
    stepSubtitleLater: "Nanti",
    stepSubtitleRequiresReview: "Perlu review",
    stepSubtitleDone: "Selesai",
    // Selected-task pane section headings
    paneProductionSummary: "Ringkasan Pesanan",
    paneCuttingInstructions: "Petunjuk Pengerjaan",
    paneProductionFiles: "File Design",
    paneStageRequirements: "Persyaratan Tahap",
    paneCountSeparator: "dari",
    paneCountComplete: "selesai",
    paneRequirementRequiredHint: "Wajib sebelum lanjut",
    paneRequirementOptionalHint: "Opsional",
    paneEnterText: "Masukkan teks...",
    paneEnterNumber: "Masukkan angka...",
    paneRequirementRemaining:
      "{count, plural, one {# persyaratan tersisa} other {# persyaratan tersisa}}",
    paneRequirementRemainingHint: "Lengkapi semua untuk melanjutkan.",
    paneCompleteRequirements: "Lengkapi Persyaratan",
    // Work-queue board short labels
    boardShortPreProd: "Pra-Prod",
    boardShortProd: "Prod",
  },
  portal: {
    title: "Lengkapi Data Pesanan",
    waitApproval: "Pesanan Anda telah dikirim. Mohon tunggu persetujuan admin.",
    orderSummary: "Ringkasan Pesanan",
    lineItems: "Item Pesanan",
    productName: "Produk",
    quantity: "Jml",
    unitPrice: "Harga Satuan",
    lineTotal: "Total",
    designName: "Nama Desain",
    designNamePlaceholder:
      "Masukkan nama desain, nama artwork, atau label referensi",
    itemNotes: "Catatan / Spesifikasi",
    itemNotesPlaceholder: "Ukuran tinggi dan lebar logo",
    attachment: "Masukkan File Desain",
    addAttachment: "Tambah lampiran",
    shippingAddress: "Alamat Pengiriman",
    orderNumber: "Nomor Pesanan",
    noShippingAddress: "Belum ada alamat pengiriman",
    orderTotal: "Total Pesanan",
    submit: "Konfirmasi Pesanan",
    submitting: "Mengirim...",
    customerInfo: "Informasi Pelanggan",
    guestName: "Nama Lengkap",
    guestNamePlaceholder: "Masukkan nama lengkap Anda",
    guestPhone: "Nomor Telepon",
    guestPhonePlaceholder: "Masukkan nomor telepon Anda",
    guestCustomer: "Tamu",
    copyLink: "Salin Tautan",
    linkCopied: "Tautan berhasil disalin!",
    downloadInvoice: "Unduh Invoice",
    contactAdmin: "Hubungi Admin via WhatsApp",
    chatOnWhatsApp: "Chat via WhatsApp",
    confirmFailed: "Pesanan tidak dapat dikonfirmasi. Coba lagi.",
    notFound: "Pesanan tidak ditemukan",
    statusDraft: "Menunggu Konfirmasi",
    statusPending: "Menunggu Persetujuan",
    statusApproved: "Disetujui",
    statusProduction: "Dalam Produksi",
    statusInDelivery: "Dalam Pengiriman",
    statusCompleted: "Selesai",
    statusCancelled: "Dibatalkan",
    required: "Kolom ini wajib diisi",
    areaRequired: "Silakan pilih area",
    paymentAlert: "Status Pembayaran",
    paymentUnpaid: "{count} Invoice belum dibayar — {amount}",
    paymentOverdue: "Terlambat",
    paymentDueSoon: "Jatuh tempo {days} hari lagi",
    paymentAllPaid: "Semua Invoice telah dibayar",
    currentStage: "Tahap Saat Ini",
    taskTimeline: "Timeline Tugas",
    lineItemDetail: "Detail Item",
    estimatedCompletion: "Estimasi selesai: {date}",
    notes: "Catatan",
    rejectedTitle: "Pesanan Ditolak",
    loadingOrder: "Memuat pesanan...",
    loadFailed: "Pesanan tidak dapat dimuat",
    loadFailedDesc:
      "Muat ulang halaman ini atau hubungi penjual jika masalah berlanjut.",
    notFoundHelp: "Periksa tautan atau minta tautan portal baru ke penjual.",
    retry: "Coba lagi",
    whatsappOrderMessage: "Halo, terkait pesanan {order}",
    pendingHelp:
      "Detail pesanan Anda sudah diterima. Kami akan meninjaunya sebelum produksi dimulai.",
    rejectedHelp:
      "Hubungi penjual untuk merevisi pesanan atau memastikan langkah berikutnya.",
    requirementsSubmitted: "Lampiran",
    noStageTransitions: "Belum ada perpindahan tahap",
    timelineQueue: "Antrian",
    timelineQueued: "Masuk dalam antrian",
    timelineStarted: "Mulai {stage}",
    timelineCompleted: "Selesai",
    timelineTransition: "{from} Selesai -> {to}",
    timelineStageFallback: "Tahap",
    timelineProductionComplete: "Selesai",
    timelineBoardTransition: "Memasuki tahap {stage}",
    timelineStatusInProgress: "Dalam proses {stage}",
    timelineStatusCompleted: "Selesai",
    timelineStatusWaitingApproval: "Menunggu persetujuan",
    timelineReadyForProduction: "Pembayaran DP",
    timelineReadyForProductionWithStage: "Antrian {stage}",

    progressOverview: "Ringkasan cepat",
    progressStatusApprovedHelp:
      "Pesanan Anda sudah disetujui. tim {org_name} sedang melakukan proses {first_preproduction_stage_name}",
    progressStatusProductionHelp:
      "Pesanan Anda dalam proses produksi, buka halaman ini secara berkala untuk melihat perkembangan pesanan anda.",
    progressStatusDeliveryHelp:
      "Pesanan Anda sedang dikirim atau disiapkan untuk serah terima.",
    progressStatusCompletedHelp:
      "Pesanan Anda selesai. Hubungi penjual jika masih perlu bantuan.",
    progressStatusFallbackHelp:
      "Pantau detail pesanan dan pembaruan produksi terbaru di sini.",
    nextStep: "Langkah berikutnya",
    nextStepPayment:
      "Selesaikan invoice yang belum dibayar agar penjual dapat melanjutkan pesanan.",
    nextStepProduction:
      "Pantau kartu item di bawah untuk melihat pembaruan tahap dari tim produksi.",
    nextStepDelivery: "Tunggu informasi serah terima pengiriman dari penjual.",
    nextStepCompleted: "Simpan halaman ini sebagai catatan pesanan Anda.",
    estimatedCompletionLabel: "Estimasi selesai",
    completedOnLabel: "Selesai pada",
    orderInfoName: "Nama",
    orderInfoPhone: "No. Telepon",
    orderInfoAddress: "Alamat",
    estimatedCompletionUnavailable: "Belum ada estimasi selesai",
    paymentSummary: "Pembayaran",
    paymentSummaryNoInvoice: "Belum ada invoice",
    paymentSummaryUnpaid:
      "{count, plural, one {# invoice belum dibayar} other {# invoice belum dibayar}} · {amount}",
    paymentSummaryAllPaid: "Semua invoice telah dibayar",
    lineItemsHelp: "Buka item untuk melihat timeline, catatan, dan lampiran.",
    productionDaysLabel:
      "{days, plural, one {# hari produksi} other {# hari produksi}}",
    invoiceDialogTitle: "Invoice",
    invoiceDialogEmpty: "Belum ada invoice",
    invoiceShipmentFee: "Biaya pengiriman",
    invoiceViewAll: "Lihat semua invoice",
    invoiceSummary: "{count} invoice · {amount}",
    invoiceSummaryUnpaid: "{count} belum dibayar · {amount}",
    // Shell + shared chrome
    skipToContent: "Lewati ke konten pesanan",
    orderPillLabel: "Pesanan",

    // Status hero + tracker
    stageTrackerLabel: "Tahap produksi",
    stageAwaiting: "Menunggu",
    stageRemaining: "{count} tahap lagi",
    stageInProgress: "Sedang berjalan",
    stageComplete: "Selesai",
    stageCurrent: "Saat ini",
    stageUpcoming: "Akan datang",
    stagesUnknown: "Belum ada pembaruan tahap",

    // Loading + error states
    loadingTitle: "Memuat pesanan Anda",
    loadingDescription: "Mengambil status terbaru dari workshop.",
    notFoundTitle: "Pesanan tidak ditemukan",
    notFoundAction: "Periksa tautan atau minta tautan baru ke penjual.",

    // Pending view
    pendingHeroTitle: "Pesanan diterima",
    pendingHeroSubtitle: "Dikirim ke {org} untuk ditinjau.",
    pendingReceivedAt: "Diterima {date}",

    // Rejected view
    rejectedHeroTitle: "Pesanan perlu direvisi",
    rejectedHeroSubtitle:
      "Gunakan catatan penjual di bawah untuk memperbaiki dan mengirim ulang.",
    rejectedReasonTitle: "Alasan dari penjual",
    rejectedReasonEmpty:
      "Tidak ada alasan diberikan. Hubungi via WhatsApp untuk detail.",
    rejectedAffectedItems: "Item terpengaruh",
    rejectedResubmitCta: "Diskusikan dengan penjual",

    // Draft view (multi-step)
    draftSubmitHelp:
      "Silakan isi data diri dan informasi pesanan Anda. Tim {org} akan segera memproses pesanan setelah data lengkap.",
    draftStepCustomer: "Beri tahu siapa Anda",
    draftStepShipping: "Mau dikirim ke mana?",
    draftStepItems: "Tinjau setiap item",
    draftStepCustomerDesc:
      "Digunakan agar penjual dapat mengonfirmasi pesanan dengan Anda.",
    draftStepShippingDesc: "Kami isi otomatis — koreksi yang belum tepat.",
    draftStepItemsDesc:
      "Ubah nama, tambahkan catatan, atau lampirkan referensi per item.",
    draftStepComplete: "Selesai",
    draftStepEmpty: "Belum diisi",
    draftStepInProgress: "Sedang diisi",
    draftGuestShort: "Tamu",

    // Progress view (status hero + obligations)
    progressHeroLabel: "Status pesanan",
    progressChecklistTitle: "Yang perlu Anda perhatikan",
    obligationPayTitle: "Selesaikan invoice yang belum dibayar",
    obligationPayBody:
      "{count, plural, one {# invoice menunggu bukti pembayaran Anda} other {# invoice menunggu bukti pembayaran Anda}}.",
    obligationNextTitle: "Selanjutnya",
    obligationNextProduction:
      "Produksi berlanjut. Halaman ini diperbarui saat item melewati tahap.",
    obligationNextDelivery:
      "Pengiriman sedang berjalan. Tunggu info serah terima kurir.",
    obligationNextCompleted:
      "Pesanan selesai. Simpan halaman ini sebagai catatan Anda.",
    itemsSectionTitle: "Daftar Pesanan",
    invoicesSectionTitle: "Invoice",
    invoicesSectionDescription:
      "Unduh invoice dan unggah bukti pembayaran di sini.",
    invoicePaidOn: "Lunas",
    invoiceDownPayment: "Down Payment",
    invoiceFinalPayment: "Pelunasan",
    invoiceUnpaidNoDue: "Belum dibayar",
    invoiceDueLabel: "Jatuh tempo {date}",
    invoiceOverdueOn: "Terlambat {date}",
    invoiceBankCopy: "Salin nomor rekening",
    invoiceBankCopied: "Tersalin",
    invoiceProofPending: "Menunggu konfirmasi",
    invoicePaymentInstructions: "Instruksi",
    invoiceShowAll: "Lihat semua ({count})",
    invoiceShowLess: "Sembunyikan",
    itemDeadlineLabel: "Deadline {date}",
    itemProductionDaysInline: "Produksi {days} hari kerja",
    itemShowTimeline: "Lihat timeline",
    itemHideTimeline: "Sembunyikan timeline",
    itemAttachmentsLabel: "Design Pesanan",
    itemNoAttachments: "Tidak ada lampiran untuk item ini",
    itemNoEvents: "Belum ada pembaruan tahap",
    rejectedNote: "Alasan: {note}",
    payNow: "Bayar Sekarang",
    paymentCancelled: "Pembayaran dibatalkan. Silakan coba lagi.",
    paymentSuccess: "Pembayaran berhasil!",
    paymentFailed: "Pembayaran gagal. Silakan coba lagi.",
    midtransSdkNotLoaded: "SDK Midtrans belum dimuat. Silakan coba lagi.",
    processingPayment: "Memproses...",
    paymentVerifying: "Memverifikasi pembayaran Anda...",
    paymentConfirmTimeout:
      "Pembayaran diterima tetapi konfirmasi membutuhkan waktu lebih lama dari biasanya. Silakan muat ulang sebentar lagi.",
    paymentAmountMismatch: "Nominal pembayaran tidak dapat diverifikasi. Silakan hubungi dukungan.",
    paymentGatewayUnavailable: "Gateway pembayaran sedang tidak tersedia. Silakan coba lagi nanti.",
    paymentManualReview: "Pembayaran Anda perlu ditinjau manual. Kami akan segera memberi kabar.",
    shipmentTracking: "Pelacakan Pengiriman",
    storePickup: "Pengambilan di Toko",
    trackShipment: "Lacak pengiriman",
    orderTimelineSectionTitle: "Riwayat Pesanan",
    timelineLastUpdatePrefix: "Update Terakhir: ",
    timelineExpand: "Lihat selengkapnya",
    timelineCollapse: "Sembunyikan",
    orderTimelineEmpty: "Belum ada pembaruan",
    timelineDraftCreated: "Draft pesanan dibuat",
    timelineDraftConfirmed: "Draft pesanan dikonfirmasi, menunggu review",
    timelineOrderApproved: "Pesanan disetujui admin",
    timelineDpInvoiceCreated: "Invoice DP dibuat",
    timelinePaymentDpConfirmed: "Pembayaran DP dikonfirmasi",
    timelineProductionStarted: "Produksi dimulai",
    timelineFinalInvoiceCreated: "Invoice pelunasan dibuat",
    timelinePaymentFinalConfirmed: "Pembayaran pelunasan dikonfirmasi",
    timelineProductionFinished: "Produksi selesai, siap dikirim",
    timelineShipmentConfirmed: "Pengiriman dikonfirmasi",
    timelineOrderCompleted: "Selesai",
    timelineQuantityAdjusted: "{product}: {oldQty} → {newQty} pcs ({impact})",
    timelineStepCompleted: "Selesai",
    timelineStepCurrent: "Saat ini",
    timelineStepUpcoming: "Menunggu",
    timelineDateUnavailable: "Tanggal belum tercatat",
    timelineShowAll: "Tampilkan seluruh proses",
    timelineShowLess: "Sembunyikan langkah mendatang",
  },
  combobox: {
    searchPlaceholder: "Cari...",
    noResults: "Hasil tidak ditemukan",
    loading: "Mencari...",
  },
  assistant: {
    trigger: "Buka asisten",
    title: "Asisten Pabriq",
    description:
      "Tanyakan pesanan, pelanggan, produk, invoice, atau tugas produksi yang bisa Anda akses.",
    attachment: "Lampiran",
    cancel: "Batal",
    copyMessage: "Salin pesan",
    emptyTitle: "Tanyakan tentang workspace Anda",
    emptyDescription:
      'Coba "Pesanan mana yang masih terbuka?" atau "Cari Acme".',
    editMessage: "Edit pesan",
    loadingHistory: "Memuat percakapan...",
    messageLabel: "Pesan",
    messagePlaceholder: "Tanyakan hal bisnis...",
    send: "Kirim",
    sending: "Mengirim...",
    you: "Anda",
    me: "Saya",
    assistant: "Asisten",
    notConfigured: "Asisten AI belum dikonfigurasi.",
    genericError: "Asisten belum bisa menjawab. Coba lagi.",
    responding: "Merespons...",
    thinking: "Berpikir...",
    generatingResponse: "Menyusun jawaban...",
    reasoning: "Penalaran",
    nextResponse: "Respons berikutnya",
    previousResponse: "Respons sebelumnya",
    regenerateResponse: "Buat ulang respons",
    rateLimitError: "Batas penggunaan tercapai. Silakan coba lagi nanti.",
    historyError: "Gagal memuat riwayat percakapan.",
    retry: "Coba lagi",
    jumpToLatest: "Lompat ke terbaru",
    loadEarlier: "Muat pesan sebelumnya",
    loadingEarlier: "Memuat pesan sebelumnya...",
    keyboardHint: "Tekan Enter untuk mengirim, Shift+Enter untuk baris baru",
    today: "Hari Ini",
    yesterday: "Kemarin",
    starterOrderDraft: "Buat draf pesanan baru",
    starterPendingWork: "Pekerjaan apa yang perlu perhatian?",
    starterPriorities: "Apa prioritas utama kita?",
    save: "Simpan",
    toolCall: {
      generic: "Memanggil {name}",
      search: "Mencari catatan",
      overview: "Memuat ringkasan kerja",
      propose: "Membuat draf pesanan",
      resolve: "Menyelesaikan item draf",
      confirm: "Membuat pesanan draf",
      running: "Sedang berjalan",
      completed: "Selesai",
      failed: "Gagal",
      cancelled: "Dibatalkan",
      actionRequired: "Perlu tindakan",
      details: "Lihat detail",
      arguments: "Argumen",
      result: "Hasil",
    },
    proposal: {
      title: "Proposal draft",
      subtotal: "Subtotal",
      total: "Total",
      customer: "Pelanggan",
      expiresIn: "{minutes} menit lagi",
      proceed: "Proses — buat draft",
      cancel: "Batal",
      cancelled: "Proposal draft dibatalkan.",
      expired:
        "Proposal ini sudah tidak berlaku. Minta asisten buat yang baru.",
    },
    error: {
      tryAgain: "Coba lagi dengan meminta asisten membuat proposal baru.",
    },
  },
  notifications: {
    title: "Notifikasi tindakan",
    pageTitle: "Notifikasi tindakan",
    description: "Alur kerja yang menunggu tindakan owner atau admin.",
    subtitle:
      "{count, plural, =0 {Tidak ada tindakan tertunda} one {# tindakan tertunda} other {# tindakan tertunda}}",
    viewAll: "Lihat semua",
    emptyTitle: "Tidak ada tindakan tertunda",
    emptyDescription:
      "Konfirmasi pembayaran, review pesanan, permintaan invoice, dan review tugas akan muncul di sini.",
    paymentConfirmations: "Konfirmasi pembayaran",
    orderReviews: "Review pesanan",
    taskReviews: "Review tugas",
    dpInvoiceRequests: "Permintaan invoice DP",
    finalInvoiceRequests: "Permintaan invoice final",
    paymentConfirmationLabel: "Konfirmasi pembayaran",
    orderReviewLabel: "Review pesanan",
    taskReviewLabel: "Review tugas",
    dpInvoiceLabel: "Kirim invoice DP",
    finalInvoiceLabel: "Kirim invoice final",
    paymentConfirmationMessage:
      "{customerName} mengirim {amount} untuk invoice {invoiceNumber}.",
    orderReviewMessage:
      "{customerName} mengirim pesanan {orderNumber} untuk direview.",
    taskReviewMessage: "{taskNumber} perlu direview di {stageName}.",
    dpInvoiceMessage:
      "Semua tugas pra-produksi untuk pesanan {orderNumber} sudah masuk antrian {stageName}. Kirim invoice DP ke {customerName} untuk melanjutkan pesanan.",
    finalInvoiceMessage:
      "Semua tugas produksi untuk pesanan {orderNumber} sudah selesai. Kirim invoice final ke {customerName}.",
    openAction: "Buka",
    amount: "Jumlah",
    noCustomer: "Pelanggan tidak diketahui",
    noOrderNumber: "Nomor pesanan belum ada",
    noTaskNumber: "Nomor tugas belum ada",
    noStage: "Tahapan tidak diketahui",
    productionStageFallback: "produksi",
  },
  operator: {
    accessDeniedTitle: "Akses ditolak",
    accessDeniedDescription: "Anda tidak memiliki akses ke halaman ini.",
    backToProduction: "Kembali ke produksi",
    backToDashboard: "Kembali ke dashboard",
    noOrgHeader: "Tanpa perusahaan",
    noOrgTitle: "Belum tergabung perusahaan",
    noOrgDescription:
      "Akun Anda belum menjadi anggota perusahaan mana pun. Hubungi administrator atau keluar.",
    theme: "Tema",
  },
  dateField: {
    pickDate: "Pilih tanggal",
    pickDateRange: "Pilih rentang tanggal",
    selectDate: "Pilih tanggal",
    startDate: "Tanggal mulai",
    endDate: "Tanggal selesai",
    clear: "Bersihkan",
    today: "Hari Ini",
    yesterday: "Kemarin",
    tomorrow: "Besok",
    nextWeek: "Minggu Depan",
    last7Days: "7 Hari Terakhir",
    last30Days: "30 Hari Terakhir",
    thisMonth: "Bulan Ini",
    lastMonth: "Bulan Lalu",
  },
  channels: {
    title: "Saluran",
    subtitle:
      "Hubungkan bot Telegram perusahaan Anda dan kendalikan siapa yang dapat berkomunikasi dan mengirimkan data.",
    tabs: {
      connections: "Koneksi",
      accounts: "Akun",
    },
    access: "Akses Saluran",
    accessDescription:
      "Hanya identitas pesan yang disetujui yang dapat mengirim pesan atau data melalui bot ini.",
    loading: "Memuat akses saluran...",
    empty:
      "Tidak ada identitas pesan yang sesuai dengan status ini. Pengguna akan muncul di sini setelah memulai bot yang terhubung.",
    error: {
      loadFailed: "Gagal memuat akun akses saluran",
    },
    telegram: {
      title: "Bot Telegram",
      connectedAs: "Terhubung sebagai {bot}.",
      connectDescription:
        "Hubungkan satu bot khusus untuk perusahaan ini. Tokennya disimpan secara aman dan tidak akan ditampilkan lagi.",
      active: "Koneksi aktif",
      inactive: "Belum terhubung",
      botId: "ID Bot {id}",
      token: "Token BotFather",
      replaceToken: "Ganti token BotFather",
      verificationNote:
        "Pabriq memverifikasi token ini dan mendaftarkan pengiriman terautentikasi sebelum menyimpannya.",
      botTokenPlaceholder: "123456789:ABCdefGHIjklMNOpqrsTUVwxyZ",
      invalidToken: "Format token bot tidak valid",
      tokenRequired: "Token wajib diisi",
      defaultBotName: "Bot Telegram",
    },
    whatsapp: {
      comingSoon: "Segera Hadir",
      description:
        "Hubungkan akun resmi WhatsApp Business Anda untuk menerima pesan dan mengelola alur kerja pelanggan.",
      notifyMe: "Belum tersedia",
    },
    status: {
      label: "Status",
      all: "Semua",
      pending: "Menunggu",
      approved: "Disetujui",
      revoked: "Dicabut",
    },
    table: {
      identity: "Identitas Pesan",
      telegramId: "ID Telegram",
      status: "Status",
      lifecycle: "Siklus Hidup",
      actions: "Tindakan",
      searchPlaceholder: "Cari akun...",
    },
    lifecycle: {
      requested: "Diminta {date}",
      approved: "Disetujui {date}",
      revoked: "Dicabut {date}",
    },
    actions: {
      disconnect: "Putuskan",
      verifying: "Memverifikasi...",
      verifyReplace: "Verifikasi dan ganti",
      verifyConnect: "Verifikasi dan hubungkan",
      approve: "Setujui",
      reapprove: "Setujui lagi",
      decline: "Tolak",
      revoke: "Cabut",
      cancel: "Batal",
      saving: "Menyimpan...",
      confirm: "Konfirmasi",
    },
    toast: {
      connected: "Bot Telegram terhubung",
      replaced: "Bot Telegram diganti",
      disconnected: "Bot Telegram diputuskan",
      approved: "{identity} disetujui",
      declined: "{identity} ditolak",
      revoked: "{identity} dicabut",
    },
    dialog: {
      disconnectTitle: "Putuskan bot Telegram?",
      approveTitle: "Setujui {identity}?",
      declineTitle: "Tolak {identity}?",
      revokeTitle: "Cabut {identity}?",
      disconnectDescription:
        "{bot} akan berhenti menerima pesan masuk. Riwayat Akses Saluran tetap terlihat.",
      approveDescription:
        "{identity} akan diberi tahu dan dapat mengirim pesan ke asisten.",
      revokeDescription:
        "{identity} akan diberi tahu dan tetap disimpan sebagai dicabut. Hanya pemilik atau administrator yang dapat menyetujui identitas ini lagi.",
    },
    telegramReplies: {
      requestSent:
        "Permintaan akses Anda telah dikirim ke administrator perusahaan untuk disetujui.",
      stillPending:
        "Permintaan akses Anda masih menunggu persetujuan dari administrator perusahaan.",
      revoked: "Akses Anda ke saluran ini telah dicabut oleh administrator.",
      welcome:
        "Selamat datang! Akses Anda telah disetujui. Anda sekarang dapat mengirim pesan dan dokumen.",
      startRequired: "Silakan kirim /start untuk meminta akses ke bot ini.",
      replaced:
        "Koneksi bot Telegram ini telah diganti dengan konfigurasi bot baru.",
      photoDownloadFailed:
        "Gagal mengunduh foto yang dilampirkan. Silakan coba kirim ulang.",
      processingFailed:
        "Maaf, terjadi kesalahan saat memproses permintaan Anda. Silakan coba lagi nanti.",
      approvedNotification:
        "Permintaan akses Anda telah disetujui! Anda sekarang dapat mengirim pesan ke bot ini.",
      revokedNotification:
        "Akses Anda ke saluran ini telah dicabut oleh administrator.",
    },
  },
};

export default id
