...
import { organization, productionStages, productionTasks } from './src/db/schema.ts'
import { eq, and } from 'drizzle-orm'

async function main() {
  // 1. Get orgId for 'labq-dev'
  const orgResult = await db.select().from(organization).where(eq(organization.slug, 'labq-dev')).limit(1)
  ...
  const stagesResult = await db.select().from(productionStages)
    .where(and(
  ...
  const tasksResult = await db.select().from(productionTasks)
    .where(and(
  ...
  const tasksWithInvalidStageId = tasksWithStageId.filter(t => !stageIds.has(t.stageId))
  ...
  console.log(`Tasks with NULL stageId: ${tasksResult.length - tasksWithStageId.length}`)
