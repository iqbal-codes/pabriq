...
//
// Usage:
//   bun --bun run .sandcastle/main.ts
// Or add to package.json:
//   "scripts": { "sandcastle": "bun --bun run .sandcastle/main.ts" }
...
// Docker provider with shared mounts (opencode credentials, etc.)
const sandboxProvider = docker({
  mounts: [
    {
      hostPath: '~/.local',
      sandboxPath: '/home/agent/.local',
      readonly: false,
    },
  ],
})
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const plan = await sandcastle.run({
    ...
    agent: sandcastle.opencode('minimax-coding-plan/MiniMax-M2.7'),
